...
public class TokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (requestUri.equals("/user/login")) { //如果是登录请求，我们不需要验证token
            //直接放行，不需要验证token
            filterChain.doFilter(request, response);
        } else {
            String token = request.getHeader("token"); //从请求头中获取token的值
            if (!StringUtils.hasText(token)) { //前面有个 “非”
                ...
                if (!verify) { //前面有个 “非”
                    Result<Object> result = Result.builder().code(902).msg("请求Token不合法").build();
                    response.getWriter().write(JSONUtil.toJsonStr(result));
                } else {
                    JSONObject payloads = JWTUtil.parseToken(token).getPayloads();
                    String userJSON = payloads.get("user", String.class);
                    UserDetailsImpl tUser = JSONUtil.toBean(userJSON, UserDetailsImpl.class);
                    ...
                    if (!token.equals(redisToken)) { //前面有个 “非”
                        Result<Object> result = Result.builder().code(903).msg("请求Token错误").build();
                        response.getWriter().write(JSONUtil.toJsonStr(result));
                    } else {
                        //token验证通过了，
